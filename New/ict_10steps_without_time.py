...
visualization_path = 'visualization/ict_10steps_without_time_feature/'
os.makedirs(visualization_path, exist_ok=True)
test_result_path = os.path.join(visualization_path, 'test_result.txt')
model_path = os.path.join(visualization_path, 'ict_10steps_without_time_feature.h5')

# -------------------------------------------------
# 2. 数据读取与基本配置
# -------------------------------------------------
# data = pd.read_csv(r"C:\Users\uceekx0\Desktop\ICLSTM_XKP\simulation_output.csv")
data = pd.read_csv('./New/simulation_output.csv')
...
class ConvexMultiHeadAttention(Layer):
    ...
    def __init__(self, d_model, num_heads, r=0.3, tau=1.0, **kwargs):
        super().__init__(**kwargs)
        self.d_model = d_model
        self.num_heads = num_heads
        assert d_model % num_heads == 0, "d_model 必须能被 num_heads 整除"
        self.head_dim = d_model // num_heads
        self.r = r  # 阈值参数，控制稀疏性（可标量或与 z 同形状）
        self.tau = tau  # 温度参数，需 > 0
...
class TransformerBlock(Layer):
    ...
    def __init__(self, d_model, num_heads, ff_dim, rate=0.1, r=0.3, tau=1.0, **kwargs):
        super().__init__(**kwargs)
        # 自定义多头自注意力层
        self.att = ConvexMultiHeadAttention(d_model=d_model, num_heads=num_heads, r=r, tau=tau)
...
def build_transformer_model(
        ...
        num_layers=2,
        dropout_rate=0.1,
        r=0.3,  # Convex-r-Softmax 参数 r
        tau=1.0  # Convex-r-Softmax 参数 tau
):
    """
    构建 IC-EoT 模型：
    ...
    for _ in range(num_layers):
        x = TransformerBlock(d_model=d_model, num_heads=num_heads, ff_dim=ff_dim,
                             rate=dropout_rate, r=r, tau=tau)(x)
...
model = build_transformer_model(
    ...
    tau=10.0  # Convex-r-Softmax 参数 tau
)

early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
optimizer = Adam(learning_rate=1e-4)  # 5e-3: 0.98
...
print(f"训练耗时: {total_time:.2f} 秒, 训练总共花费： {total_epochs:.2f} epochs")
model.summary()
